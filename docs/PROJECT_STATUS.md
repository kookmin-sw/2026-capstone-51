...
### 백엔드 IP 갱신: 44.222.126.118 (2026-05-13, EC2 재재시작 ×2)

- **사유**: 같은 날 EC2 두 번 재시작. `3.238.28.206` → `3.238.245.5` → `44.222.126.118`.
- **변경**: `vite.config.js` 의 `API_TARGET` 갱신(사용자 직접 수정), `frontend/CLAUDE.md` 의 IP 안내 갱신, `docs/PROJECT_STATUS.md` 본문 옛 IP 직접 참조 일괄 치환, dev 서버 재시작.
- **항목 6번 (Elastic IP 할당) 재강조 필요** — 같은 작업 반복 (이번이 4회째 관찰, 하루 두 번).
