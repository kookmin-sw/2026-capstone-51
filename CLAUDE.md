...
2. **API base path는 `/api`**: 백엔드 `server.servlet.context-path: /api`. 프론트 `VITE_API_URL`은 `https://api.logi.p-e.kr/api` (배포 백엔드) 또는 `http://localhost:8080/api` (로컬 백엔드)처럼 `/api`까지 포함해야 함.
